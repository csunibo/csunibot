...
const { LoadDebugListeners } = require("../util/debug");
...
class InfoBot extends Client {
	...
	build() {
		...
		// Enables in depth logging for inspection
		if (this.config.devDebug === true) {
			this.error("DEV mode [ENABLED]!");
	...
	LoadEvents(config) {
		...
		fs.readdir(EventsDir, (err, files) => {
			...
			this.info("Event listeners have been bound")
		});
	}
	...
	LoadSchedules(config) {
		...
		fs.readdir(SchedulesDir, (err, files) => {
			...
			this.info("Schedules have been bound");
		});
	}
