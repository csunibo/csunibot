...
module.exports = (client) => {
	...
	client.warn("Loading Music Client...");
	...
	.on("nodeConnect", (node) =>
	client.log(`Node: ${node.options.identifier} | Connected.`))
	
	.on("nodeReconnect", (node) =>
	client.warn(`Node: ${node.options.identifier} | Reconnecting.`))
	
	.on("nodeDestroy", (node) =>
	client.warn(`Node: ${node.options.identifier} | Destroyed.`))
	
	.on("nodeDisconnect", (node) =>
	client.warn(`Node: ${node.options.identifier} | Disconnected.`))
