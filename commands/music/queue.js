...
.setRun(async (client, interaction, options) => {
	...
	if (!interaction.member.voice.channel) {
		...
		.setColor(client.config.embedColor)
		.setDescription("You have to join voice channel before you can use this command");
	...
	if (!player.queue.size || player.queue.size === 0) {
		...
		.addFields({
			name: "Duration",
			value: `\`${pms(player.position, { colonNotation: true })} / ${pms(player.queue.current.duration, { colonNotation: true })}\``,
			inline: true,
		},
		{
			name: "Volume",
		...
		const mapping = player.queue.map((t, i) => `\` ${++i} \` [${t.title}](${t.uri}) [${t.requester}]`);
		...
		if (player.queue.size < 5 || player.queue.totalSize < 11) {
			let song = player.queue.current;
			const embedTwo = new MessageEmbed()
			.setColor(client.config.embedColor)
			.setDescription(`**♪ | Now playing:** [${song.title}](${song.uri}) [${player.queue.current.requester}]\n\n**Queued Tracks**\n${pages[page]}`)
			.addFields({
				name: "Track Duration",
			...
			.editReply({
				embeds: [embedThree],
				components: [new MessageActionRow().addComponents([buttonTwo, buttonOne])],
			})
			.catch(() => {});
