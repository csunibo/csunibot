	...
	if (res.loadType == "NO_MATCHES") {
		...
		let choosenTracks = await interaction.editReply({
			embeds: [
				...
				.setDescription(`Here are some of the results I found for \`${search}\`. Please select track within \`30 seconds\``),
			],
			components: [menus],
		});
		...
		tracksCollector.on("end", async (i) => {
			if (i.size == 0) {
				choosenTracks.edit({
					...
					embeds: [
						new MessageEmbed()
						.setDescription(`No track selected. You took too long to select a track.`)
						.setColor(client.config.embedColor),
					],
					components: [],
				});
